...
# Q1: The ITT (intention-to-treat) estimand in a platform A/B test is the average difference in outcomes between all users assigned to treatment versus 
# all users assigned to control, regardless of whether the feature actually delivered or was used. ITT is the default in real A/B testing because 
# it tells you the true impact of launching the feature in the real world, including delivery failures and low usage.However, a research audience may 
# instead want the effect of actually receiving/using the treatment (e.g., treatment-on-the-treated or complier average causal effect). 
# To identify that alternative estimand, you need additional assumptions such as random assignment as an instrument, plus reliable measurement of compliance/engagement.
...
os.chdir("/Users/songtao/Dropbox/26SP/SODA 501/soda501_ps/05_experiments")
print(os.getcwd())
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)s | %(message)s",  # This controls how each log message looks.
    handlers=[
