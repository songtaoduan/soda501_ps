...
fit_days.summary2().tables[1].to_csv("outputs/tables/regression_retention")
fit_ret.summary2().tables[1].to_csv("outputs/tables/regression_days.csv")
...
# Q3: Add a retention-style outcome and estimate its ATE.

# Retention: count number of active days (ignore missing days)
ret = (
    logs.loc[logs["active"].notna()]
...
# Users with zero observed active days should be treated as 0
user["days_active"] = user["days_active"].fillna(0)

# retained_any = 1 if days_active >= 1 else 0
user["retained_any"] = (user["days_active"] >= 1).astype(int)


# --- Retention ATEs (difference in means) ---
ate_days_active = (
    user.loc[user["treat"] == 1, "days_active"].mean()
    - user.loc[user["treat"] == 0, "days_active"].mean()
)

ate_retained_any = (
    user.loc[user["treat"] == 1, "retained_any"].mean()
    - user.loc[user["treat"] == 0, "retained_any"].mean()
)

# Regression adjustment with block FE + clustered SEs
fit_days_active = smf.ols(
    "days_active ~ treat + C(block)",
    data=user
).fit(cov_type="cluster", cov_kwds={"groups": user["cluster_id"]})

fit_retained_any = smf.ols(
    "retained_any ~ treat + C(block)",
    data=user
).fit(cov_type="cluster", cov_kwds={"groups": user["cluster_id"]})

# svae output tables
ate_retention = pd.DataFrame({
    "outcome": ["days_active", "retained_any"],
    "ate_diff_in_means": [ate_days_active, ate_retained_any],
    "ate_regression": [
        fit_days_active.params["treat"],
        fit_retained_any.params["treat"]
    ],
    "se_clustered": [
        fit_days_active.bse["treat"],
        fit_retained_any.bse["treat"]
    ],
    "p_value": [
        fit_days_active.pvalues["treat"],
        fit_retained_any.pvalues["treat"]
    ]
})
...
# Q4: Simulate noncompliance and compare ITT vs TOT (IV)
...
# compute ITT vs TOT (IV) and save CSV
from statsmodels.sandbox.regression.gmm import IV2SLS

# -----------------
#  ITT vs TOT (IV)
# -----------------
# Outcome to use (pick one; converted is typical)
y = user["converted"]

# ITT: regress outcome on treat (with block FE, cluster-robust SEs)
fit_itt = smf.ols(
    "converted ~ treat + C(block)",
    data=user
).fit(cov_type="cluster", cov_kwds={"groups": user["cluster_id"]})

# TOT/LATE via IV: treat instruments received
block_d = pd.get_dummies(user["block"], prefix="block", drop_first=True)

X = pd.concat(
    [
        pd.Series(1.0, index=user.index, name="const"),
        user[["received"]],
        block_d
    ],
    axis=1
...
# ---- IMPORTANT: force numeric + drop any NA rows ----
y_iv = pd.to_numeric(y, errors="coerce").astype(float)
X_iv = X.apply(pd.to_numeric, errors="coerce").astype(float)
Z_iv = Z.apply(pd.to_numeric, errors="coerce").astype(float)

ok = y_iv.notna() & X_iv.notna().all(axis=1) & Z_iv.notna().all(axis=1)

y_iv = y_iv.loc[ok]
X_iv = X_iv.loc[ok]
Z_iv = Z_iv.loc[ok]

# Fit IV USING the numeric arrays (this is the key fix)
iv = IV2SLS(y_iv.to_numpy(), X_iv.to_numpy(), Z_iv.to_numpy()).fit()

# Cluster-robust SEs (after fit) — use the same ok rows
iv_cl = iv.get_robustcov_results(
    cov_type="cluster",
    groups=user.loc[ok, "cluster_id"].to_numpy()
)

# First stage / take-up
takeup_treated = user.loc[user["treat"] == 1, "received"].mean()
...
tot_se   = iv_cl.bse[1]
tot_p    = iv_cl.pvalues[1]

out = pd.DataFrame([{
    "outcome": "converted",
    "p_receive": p_receive,
    "takeup_treated_mean": takeup_treated,
    "takeup_control_mean": takeup_control,
    "first_stage": first_stage,
    "ITT_coef_treat": fit_itt.params["treat"],
    "ITT_se_cluster": fit_itt.bse["treat"],
    "ITT_p_value": fit_itt.pvalues["treat"],
    "TOT_coef_received": tot_beta,
    "TOT_se_cluster": tot_se,
    "TOT_p_value": tot_p,
}])
...
## contribute outcomes similar to control, pulling the average effect toward zero. 
## The IV/TOT (LATE) rescales by the take-up rate (the first stage) and estimates
## the causal effect among compliers—those whose receipt is changed by assignment.

###############################################
# Q 5: MULTIPLE OUTCOMES + MULTIPLE TESTING (BH/FDR)
###############################################

logging.info("Q5: Multiple outcomes + BH/FDR")

# Ensure "converted" exists (real effect outcome)
if "converted" not in user.columns:
    user["converted"] = (user["post_purchases"] > 0).astype(int)

np.random.seed(123)  

# 1) Real outcome with true effect: use an existing outcome
user["outcome_real"] = user["converted"]

# 2) 9 placebo outcomes: depend on covariates but NOT on treat
#    (Use baseline_activity, pre_metric, and platform effects + noise)
#    No treat term included anywhere.
user["placebo_1"] = 0.5*np.log1p(user["baseline_activity"]) + 0.3*user["pre_metric"] + np.random.normal(0, 1.0, size=len(user))
user["placebo_2"] = 0.2*np.log1p(user["baseline_activity"]) - 0.1*user["pre_metric"] + 0.5*(user["platform"]=="ios").astype(float) + np.random.normal(0, 1.0, size=len(user))
user["placebo_3"] = 0.1*np.log1p(user["baseline_activity"]) + 0.4*(user["platform"]=="android").astype(float) + np.random.normal(0, 1.0, size=len(user))
user["placebo_4"] = 0.3*user["pre_metric"] + 0.2*(user["platform"]=="web").astype(float) + np.random.normal(0, 1.0, size=len(user))
user["placebo_5"] = 0.6*np.log1p(user["baseline_activity"]) + np.random.normal(0, 1.0, size=len(user))
user["placebo_6"] = -0.2*np.log1p(user["baseline_activity"]) + 0.2*user["pre_metric"] + np.random.normal(0, 1.0, size=len(user))
user["placebo_7"] = 0.15*user["pre_metric"] + np.random.normal(0, 1.0, size=len(user))
user["placebo_8"] = 0.25*np.log1p(user["baseline_activity"]) + 0.1*(user["platform"]=="ios").astype(float) + np.random.normal(0, 1.0, size=len(user))
user["placebo_9"] = 0.05*np.log1p(user["baseline_activity"]) - 0.2*(user["platform"]=="android").astype(float) + np.random.normal(0, 1.0, size=len(user))

outcome_names = ["outcome_real"] + [f"placebo_{k}" for k in range(1, 10)]

# For each outcome: estimate treat effect with block FE + cluster robust SE
rows = []
for nm in outcome_names:
    fit = smf.ols(
        f"{nm} ~ treat + baseline_activity + pre_metric + C(block)",
        data=user
    ).fit(cov_type="cluster", cov_kwds={"groups": user["cluster_id"]})
...
p_sorted = p[order]
bh_sorted = p_sorted * m / np.arange(1, m + 1)
...
res["bh_p_value"] = bh
res["significant_q05"] = (res["bh_p_value"] <= 0.05).astype(int)

# Nice ordering: real first, then placebos
res["outcome_order"] = res["outcome"].replace(
    {"outcome_real": "0_outcome_real"}
)
res["outcome_order"] = res["outcome_order"].apply(lambda s: s.replace("placebo_", "1_placebo_"))
res = res.sort_values("outcome_order").drop(columns=["outcome_order"])

# Save
res.to_csv("outputs/tables/multiple_testing.csv", index=False)
